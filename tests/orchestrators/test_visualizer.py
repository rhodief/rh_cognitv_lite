...
class TestTerminalFormat:
    def test_returns_non_empty_string(self, linear_dag):
        viz = DAGVisualizer(linear_dag)
        output = viz._render_terminal()
        assert isinstance(output, str) and output.strip()

    def test_contains_all_node_ids(self, linear_dag):
        output = DAGVisualizer(linear_dag)._render_terminal()
        for id_ in ("a", "b", "c"):
            assert id_ in output

    def test_contains_group_node_id(self, grouped_dag):
        output = DAGVisualizer(grouped_dag)._render_terminal()
        assert "grp" in output

    def test_expands_group_inner_nodes(self, grouped_dag):
        output = DAGVisualizer(grouped_dag)._render_terminal()
        # inner nodes of the group must appear
        assert "p" in output
        assert "q" in output

    def test_deeply_nested_inner_nodes_present(self, nested_dag):
        output = DAGVisualizer(nested_dag)._render_terminal()
        for id_ in ("og", "ig", "x", "y", "z", "final"):
            assert id_ in output

    def test_group_tag_appears(self, grouped_dag):
        output = DAGVisualizer(grouped_dag)._render_terminal()
        assert "[Group]" in output

    def test_node_tag_appears(self, linear_dag):
        output = DAGVisualizer(linear_dag)._render_terminal()
...
class TestJSONFormat:
    def test_json_output_is_valid_dict(self, linear_dag):
        result = DAGVisualizer(linear_dag)._render_json()
        assert isinstance(result, dict)

    def test_json_has_nodes_and_edges_keys(self, linear_dag):
        result = DAGVisualizer(linear_dag)._render_json()
        assert "nodes" in result and "edges" in result

    def test_json_contains_all_node_ids(self, linear_dag):
        result = DAGVisualizer(linear_dag)._render_json()
        ids = {node["id"] for node in result["nodes"]}
        assert ids == {"a", "b", "c"}

    def test_json_contains_edges(self, linear_dag):
        result = DAGVisualizer(linear_dag)._render_json()
        pairs = {(e["source"], e["target"]) for e in result["edges"]}
        assert ("a", "b") in pairs
        assert ("b", "c") in pairs

    def test_json_edge_label_included(self):
        a, b = n("a"), n("b")
        dag = DAGBuilder().node(a).node(b).edge("a", "b", label="my-label").build()
        result = DAGVisualizer(dag)._render_json()
        edge = next(e for e in result["edges"] if e["source"] == "a")
        assert edge["label"] == "my-label"

    def test_json_edge_no_label_key_omitted(self, linear_dag):
        result = DAGVisualizer(linear_dag)._render_json()
        for edge in result["edges"]:
            assert "label" not in edge

    def test_json_group_node_has_inner_key(self, grouped_dag):
        result = DAGVisualizer(grouped_dag)._render_json()
        grp_entry = next(node for node in result["nodes"] if node["id"] == "grp")
        assert "inner" in grp_entry

    def test_json_group_inner_contains_nodes(self, grouped_dag):
        result = DAGVisualizer(grouped_dag)._render_json()
        grp_entry = next(node for node in result["nodes"] if node["id"] == "grp")
        inner_ids = {nd["id"] for nd in grp_entry["inner"]["nodes"]}
        assert inner_ids == {"p", "q"}

    def test_json_group_inner_contains_edges(self, grouped_dag):
        result = DAGVisualizer(grouped_dag)._render_json()
        grp_entry = next(node for node in result["nodes"] if node["id"] == "grp")
        inner_pairs = {
    ...
    def test_json_plain_node_has_no_inner_key(self, grouped_dag):
        result = DAGVisualizer(grouped_dag)._render_json()
        start = next(node for node in result["nodes"] if node["id"] == "start")
        assert "inner" not in start

    def test_json_nested_group_topology(self, nested_dag):
        result = DAGVisualizer(nested_dag)._render_json()
        og = next(node for node in result["nodes"] if node["id"] == "og")
        # og has inner; inner has ig node with its own inner
    ...
    def test_json_render_with_indent_option(self, linear_dag, capsys):
        ...
        captured = capsys.readouterr()
        # 4-space indent produces lines with 4 spaces
        assert "    " in captured.out

    def test_json_metadata_included(self):
        node_with_meta = Node(
...
class TestDAGVisualizPassthrough:
    ...
    def test_dag_visualize_dict_format(self, linear_dag, capsys):
        linear_dag.visualize({"format": "json", "indent": 2})
        captured = capsys.readouterr()
        parsed = json.loads(captured.out)
        assert "nodes" in parsed
